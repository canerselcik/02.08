...
const upload = multer({ storage: multerStorage }).single("pb_file"); //pb file 
...
/* let categories find sorgusu yapar */
...
let body = req.body;
    try{
        ...
        emitter.getEmitter("notifications").emit("messages", {message: category.name + "is added"}); // olay gerçekleşince yayın yapar events de ki dinler.
        

        res.json(Response.successResponse({success:true}));

    } catch (err){
        logger.error(req.user?.email,"Categories","Add",err); // bu hatayı kim aldı kim hangi işlemi yaparken aldı
        let errorResponse = Response.errorResponse(err);
...
router.post("/export",auth.checkRoles("category_export"),async (req, res) => {
    try {
        ...
        let excel = excelExport.toExcel(
            ...
            ["name","is_active", "created_by","created_at","updated_at"], // excelini oluşturacak kategori çağırıldıgında
            categories
        )
