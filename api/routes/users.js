...
router.post("/register",async (req, res)=>{
 ...
 try {
  ...
  let user = await Users.findOne({}); //await datayı bulmayı sağlar.

  if(user){
     return res.sendStatus(Enum.HTTP_CODES.NOT_FOUND); // eğer bir kullanıcı varsa bu endpoint yok gibi davranacak. return cannot kullandıgımız için kullanıldı.
  }

  if(!body.email) throw new CustomError(Enum.HTTP_CODES.BAD_REQUEST, "Validation Error!", "email field must be filled");
  
  if(is.not.email(body.email)) throw new CustomError(Enum.HTTP_CODES.BAD_REQUEST, "Validation Error!", "email field must be an email format"); // email eğer email formatında gelmiyorsa hata gelmesini sağlıyor.
  ...
  let password = bcrypt.hashSync(body.password, bcrypt.genSaltSync(8),null); //Bu tür bir şifreleme, kullanıcı şifrelerinin güvenliğini sağlamak için yaygın olarak kullanılır. Şifrelerin düz metin olarak saklanmaması, olası bir veri sızıntısı durumunda kullanıcıların şifrelerinin ele geçirilmesini zorlaştırır.
  ...
  let createdUser = await Users.create({
    ...
    phone_number: body.phone_number   //ilk kullanıcı
  });
  ...
  res.status(errorResponse.code).json(errorResponse);   // ERROR RESPONS
 }
...
router.post("/auth", async (req, res) => {
  try{
    ...
    let payload = { // postman üzerinden user auth yapıldıgında token geliyor tokenin içindeki değeri jwt.io ya girip ayıklayınca bize id ve exp veriyor.
      id: user._id,
...
router.all("*",auth.authenticate(), (req, res, next) =>{
  next();  // endpoint koruma register ve authu üste alma sebebimiz onları korumamıza gerek yok.
});  
...
try {
 ...
 res.json(Response.successResponse(users));
  ...
  let errorResponse = Response.errorResponse(err);
  res.status(errorResponse.code).json(errorResponse); //böylece user tablosundan veriyi okuyup bunu db ye gönderdik.

}
}); // router içerisinde get isteği gelirse buraya gel
...
router.post("/update",auth.checkRoles("user_update"),async (req, res) => {
  ...
  try {
    ...
    if(Array.isArray(body.roles) && body.roles.length > 0){
      ...
      let deleteManydRoles = userRoles.filter(x => !body.roles.includes(x.role_id)); // body.permision içinde yoksa bunu deleteManyd permissona ata.
      let newRoles = body.roles.filter(x => !userRoles.map(r => r.role_id).includes(x));

      if (deleteManydRoles.length > 0) {
        await UserRoles.deleteMany({_id : {$in : deleteManydRoles.map(x => x._id)}}); //id değeri deleteManyd permssion dizisindeki id lerden biriyse onu sil
    }
...
//cannot set headres after they are sent to the client hatası bilidnik bir hatadır, bir tane req 2 tane resp dönüyoruz.